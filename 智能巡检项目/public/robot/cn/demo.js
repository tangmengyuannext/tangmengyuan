...
$(document).ready(function(){
    console.log(window.top.sessionStorage.getItem("getrobotcameraip"),window.top.sessionStorage.getItem("getrobotcameraip") !== "null");
    if(window.top.sessionStorage.getItem("getrobotcameraip") !== "null"){
        cameraip = window.top.sessionStorage.getItem("getrobotcameraip");
        ...
        console.log(cameraip,'--cameraip--',cameraport,'--cameraport--',camerauser,'--camerauser--',camerapassword,'--camerapassword--');
        // 获取摄像头IP，直接赋值登录
        if(cameraip != null){
...
// --------------------机器人巡检-----------------------------
var robotip = "" , robotId = "";
// console.log(window.top.sessionStorage.getItem("getrobotip"),'----机器人IP','205');
console.log(window.top.sessionStorage.getItem("getrobotip"),'----机器人IP','207');
...
function clickGoPreset() {
    ...
    if (oWndInfo != null) {
        WebVideoCtrl.I_GoPreset(iPresetID, {
            ...
            error: function (status, xmlDoc) {
                console.log(oWndInfo.szDeviceIdentify + " 调用预置点失败！", status, xmlDoc);
            }
        });
    }
}
...
function clickGetOSDTime() {
    ...
    if (oWndInfo != null) {
        var szTime = WebVideoCtrl.I_GetOSDTime({
            ...
            error: function () {
                console.log(oWndInfo.szDeviceIdentify + " 获取OSD时间失败！");
            }
        });
    }
}
...
function clickImportDeviceConfig() {
    ...
    if (0 == iRet) {
        WebVideoCtrl.I_Restart(szDeviceIdentify, {
            ...
            error: function (status, xmlDoc) {
                console.log(szDeviceIdentify + " 重启失败！", status, xmlDoc);
            }
        });
    } else {
        console.log(szDeviceIdentify + " 导入失败！");
    }
}
...
function getUpgradeStatus(szDeviceIdentify) {
    ...
    if (iStatus == 0) {
        ...
        console.log(szDeviceIdentify + " 语言不匹配！");
        clearInterval(g_tUpgrade);
        g_tUpgrade = 0;
    } else {
        mWebVideoCtrl.I_StopUpgrade();
        console.log(szDeviceIdentify + " 获取状态失败！");
        clearInterval(g_tUpgrade);
        g_tUpgrade = 0;
    }
}
...
function clickRestoreDefault() {
    ...
    WebVideoCtrl.I_RestoreDefault(szDeviceIdentify, szMode, {
        ...
        error: function (status, xmlDoc) {
            console.log(szDeviceIdentify + " 恢复默认参数失败！", status, xmlDoc);
        }
    });
}
...
function clickAddSnapPolygon() {
    ...
    if (0 === iRet) {
        console.log("窗口" + g_iWndIndex + "添加图形成功！");
    } else if (-1 === iRet) {
        console.log("窗口" + g_iWndIndex + "添加图形失败！");
    } else if (-2 === iRet) {
        alert("参数错误！");
    } else if (-3 === iRet) {
        alert("图形个数达到上限！");
...
function clickEditSnapPolygon() {
    ...
    if (0 === iRet) {
        console.log("窗口" + g_iWndIndex + "编辑图形成功！");
    } else {
        console.log("窗口" + g_iWndIndex + "编辑图形失败！");
    }
}
...
function clickStopSnapPolygon() {
    ...
    if (0 === iRet) {
        console.log("窗口" + g_iWndIndex + "停止编辑成功！");
    } else {
        console.log("窗口" + g_iWndIndex + "停止编辑失败！");
    }
}
...
function clickDelAllSnapPolygon() {
    ...
    if (0 === iRet) {
        console.log("窗口" + g_iWndIndex + "清空图形成功！");
    } else {
        console.log("窗口" + g_iWndIndex + "清空图形失败！");
    }
}
