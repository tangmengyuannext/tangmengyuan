    ...
    mySelect.prototype={
        ...
        closeSelectEvent:function(){
            ...
            this.ele.find(".inputWrap").on("click",function(event){
                ...
                if(that.ele.find(".inputWrap>i").hasClass("fa-angle-down")){
                    ...
                    that.ele.find(".mySelect-option").animate({height:"300px",opacity:"0.9"},"fast","swing")
                }else{
        ...
        initOption: function () {
            ...
            for(var i= 0;i<this.options.option.length;i++){
                this.ele.find(".mySelect-option").append('<div data-value="'+this.options.option[i].value+'"><span>'+this.options.option[i].label+'</span></div>')
            }
        },
        addEvent:function(){
            var that=this;
